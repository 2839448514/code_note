...
1. **左值（lvalue）**：
    - 左值是指可以出现在赋值操作符左侧的表达式，通常表示一个对象或函数。是非亡值的泛左值；
    - 左值可以被取地址（即使用`&`操作符），并且可以被修改。
    - 左值可以进一步细分为泛左值（glvalue），泛左值包括左值和亡值（xvalue）。
2. **纯右值（prvalue）**：
    - 纯右值是指不能出现在赋值操作符左侧的表达式，通常表示一个临时对象或计算结果。
    - 纯右值不能被取地址，也不能被修改。
    - 纯右值可以进一步细分为纯右值（prvalue）和亡值（xvalue）。
3. **亡值（xvalue）**：
    - 亡值是指即将被销毁的对象，通常通过移动语义（move semantics）来处理。是代表其资源能够被重新使用的对象或位域的泛左值；
    - 亡值可以被取地址，但不能被修改。
    - 亡值可以进一步细分为泛左值（glvalue），泛左值包括左值和亡值。
4. **泛左值（glvalue）**：
    - 泛左值是左值和亡值的统称，表示一个对象或函数。是其求值确定一个对象、位域或函数的个体的表达式；
    - 泛左值可以被取地址，但不能被修改。
5. **右值（rvalue）**：
    - 右值是纯右值和亡值的统称，表示一个临时对象或计算结果。
    - 右值不能被取地址，也不能被修改。是纯右值或者亡值。
...
- **泛左值**：常用于赋值、引用、解引用等操作。例如：
  ...
  `int a = 5; // a 是一个泛左值   int& ref = a; // ref 是对 a 的引用`
